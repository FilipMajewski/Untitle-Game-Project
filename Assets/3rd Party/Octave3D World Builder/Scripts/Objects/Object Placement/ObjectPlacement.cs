...
namespace O3DWB
{
    ...
    public class ObjectPlacement : ScriptableObject, IMessageListener
    {
        ...
        public ObjectPlacementSettings Settings
        {
            get
            {
                if (_settings == null) _settings = Octave3DWorldBuilder.Instance.CreateScriptableObject<ObjectPlacementSettings>();
                return _settings;
            }
        }
        public ObjectPivotPointsRenderSettings GuidePivotPointsRenderSettings
        {
            get
            {
                if (_guidePivotPointsRenderSettings == null) _guidePivotPointsRenderSettings = Octave3DWorldBuilder.Instance.CreateScriptableObject<ObjectPivotPointsRenderSettings>();
        ...
        public ObjectVertexSnapSessionRenderSettings ObjectVertexSnapSessionRenderSettings
        {
            get
            {
                if (_objectVertexSnapSessionRenderSettings == null) _objectVertexSnapSessionRenderSettings = Octave3DWorldBuilder.Instance.CreateScriptableObject<ObjectVertexSnapSessionRenderSettings>();
        ...
        public DecorPaintObjectPlacement DecorPaintObjectPlacement
        {
            get
            {
                if (_decorPaintObjectPlacement == null) _decorPaintObjectPlacement = Octave3DWorldBuilder.Instance.CreateScriptableObject<DecorPaintObjectPlacement>();
        ...
        {
            return Octave3DWorldBuilder.Instance.ObjectPlacement;
        }
        #endregion
        ...
        public void HandleKeyboardButtonDownEvent(Event e)
        {
            ...
            if(AllShortcutCombos.Instance.StepXZGridUp.IsActive())
            {
                ...
                Octave3DWorldBuilder.Instance.Inspector.Repaint();
            }
            else
            ...
            if (ObjectPlacementGuide.ExistsInSceneAndIsActive)
            {
                ...
                if (AllShortcutCombos.Instance.CycleThroughProjectedBoxFaceGuidePivotPoints.IsActive())
                {
                    e.DisableInSceneView();
        ...
        public void HandleMouseScrollWheelEvent(Event e)
        {
            ...
            if (AllShortcutCombos.Instance.CycleThroughPrefabsInActiveCategoryUsingMouseScrollWheel.IsActive() && ObjectPlacementGuide.ExistsInScene)
            {
                ...
                if (Mathf.Abs(e.delta.y) <= 3.0f)
                {
                    ...
                    Octave3DWorldBuilder.Instance.PrefabManagementWindow.Repaint();
                }
            }
            else
        ...
        private void RespondToMessage(ObjectHierarchyRootsWerePlacedInSceneMessage message)
        {
            ...
            if(placedObjects != null && placedObjects.Count != 0)
            {
                ...
                if (CanAttachPlacedRootsAsChildrenOfActiveObjectGroups(message.ObjectPlacementType))
                {
                    GameObject activeGroupParent = GetActiveObjectGroup().GroupParent;
        ...
        {
            return Octave3DWorldBuilder.Instance.PlacementObjectGroupDatabase.ActiveGroup;
        }
        ...
        private bool CanAttachPlacedRootsAsChildrenOfHoveredObject(ObjectHierarchyRootsWerePlacedInSceneMessage.PlacementType objectPlacementType)
        {
            ...
            GameObject surfaceObject = ObjectPlacementSurfaceInfo.GetSurfaceObject();
            return Settings.MakePlacedObjectsChildrenOfHoveredObject && surfaceObject != null && Octave3DWorldBuilder.Instance.IsWorkingObject(surfaceObject);
        }

        private bool CanAttachPlacedRootsAsChildrenOfActiveObjectGroups(ObjectHierarchyRootsWerePlacedInSceneMessage.PlacementType objectPlacementType)
        {
            ObjectGroup activeObjectGroup = GetActiveObjectGroup();
            if (activeObjectGroup == null || activeObjectGroup.GroupParent == null) return false;
            ...
            return Settings.AttachPlacedObjectsToActiveObjectGroup;
        }
        ...
        private void RenderGizmosForNonDecorPaintMode()
        {
            if(!_objectVertexSnapSession.IsActive && !AllShortcutCombos.Instance.SnapXZGridToCursorPickPointOnLeftClick.IsActive())
            {
                ...
                _guidePivotPointsRenderer.RenderGizmos(_projectedGuidePivotPoints, GuidePivotPointsRenderSettings);
