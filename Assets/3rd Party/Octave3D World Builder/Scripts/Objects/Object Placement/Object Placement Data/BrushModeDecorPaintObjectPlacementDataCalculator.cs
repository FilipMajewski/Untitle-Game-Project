...
namespace O3DWB
{
    ...
    public class BrushModeDecorPaintObjectPlacementDataCalculator
    {
        ...
        public List<ObjectPlacementData> Calculate(Quaternion rotationToApplyForStrokeAlignment)
        {
            ...
            {
                // For each brush element, generate an object
                for (int brushElementIndex = 0; brushElementIndex < _allValidBrushElements.Count &&
                                                currentObjectIndex < _workingBrush.MaxNumberOfObjects; ++brushElementIndex)
                {
                    // Store brush element for easy access and increment the object counter. Even if the brush element
                    // will not be used for placement (e.g. missed, slope mask etc), we will treat it as an object which
                    // was processed.
                    DecorPaintObjectPlacementBrushElement brushElement = _allValidBrushElements[brushElementIndex];
                    ++currentObjectIndex;

                    // We will not go further if the element is missed
                    float elementMissChance = brushElement.MissChance;
                    if (ObjectPlacementMissChance.Missed(elementMissChance, DecorPaintObjectPlacementBrushElement.MinMissChance, DecorPaintObjectPlacementBrushElement.MaxMissChance)) continue;
                    ...
                    if (_objectNodeNetwork.NumberOfNodes == 0)
                    {
                        ...
                        Vector3 randomPositionInsideCircle = _workingBrushCircle.Center;
                        ...
                        if (passesSlopeTest && DoesBoxPassObjectIntersectionTest(matrixObjectBoxPair.ObjectBox)) objectPlacementDataInstances.Add(new ObjectPlacementData(matrixObjectBoxPair.ObjectMatrix, _allValidBrushElements[brushElementIndex].Prefab));
                    }
                    else
                    {
                        ...
                        if (_objectNodeNetwork.NumberOfSegments != 0)
                        {
                            ...
                            AdjustObjectBoxRotationOnSurface(objectBox, objectSurfaceData, brushElementIndex);
                            ...
                            if (passesSlopeTest && DoesBoxPassObjectIntersectionTest(finalBox)) objectPlacementDataInstances.Add(new ObjectPlacementData(objectMatrix, _allValidBrushElements[brushElementIndex].Prefab));
        ...
        private MatrixObjectBoxPair CalculateMatrixObjectBoxPair(int brushElementIndex, ObjectSurfaceData objectSurfaceData)
        {
            ...
            if (brushElement.AlignToSurface) AdjustObjectBoxRotationOnSurface(objectBox, objectSurfaceData, brushElementIndex);
  
            // The final step is to rotate the object by a random amount around the placement surface
            if(brushElement.RandomizeRotation)
            {
                if(!brushElement.AlignToStroke)
                {
                    ...
                    Quaternion additionalRotation = Quaternion.AngleAxis(randomRotationAngle, objectSurfaceData.SurfaceNormal);
        ...
        private void AdjustObjectBoxRotationOnSurface(OrientedBox objectBox, ObjectSurfaceData objectSurfaceData, int brushElementIndex)
        {
            DecorPaintObjectPlacementBrushElement brushElement = _allValidBrushElements[brushElementIndex];
        ...
        private bool DoesBoxPassObjectIntersectionTest(OrientedBox objectBox)
        {
            return _allowObjectIntersection || !ObjectQueries.IntersectsAnyObjectsInScene(objectBox, new List<GameObject> { DecorPaintObjectPlacement.Get().DecorPaintSurfaceObject}, true);
        }
        #endregion
    }
}
