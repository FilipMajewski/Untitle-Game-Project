...
namespace O3DWB
{
    ...
    public class ObjectSnapping : ScriptableObject
    {
        ...
        public ObjectSnapSettings Settings
        {
            get
            {
                if (_settings == null) _settings = Octave3DWorldBuilder.Instance.CreateScriptableObject<ObjectSnapSettings>();
                return _settings;
            }
        }
        public XZGrid XZSnapGrid
        {
            get
            {
                if (_xzSnapGrid == null) _xzSnapGrid = Octave3DWorldBuilder.Instance.CreateScriptableObject<XZGrid>();
                return _xzSnapGrid;
            }
        }
        ...
        {
            return Octave3DWorldBuilder.Instance.ObjectSnapping;
        }
        #endregion
        ...
        public void SnapObjectHierarchy(GameObject hierarchyRoot, ProjectedBoxFacePivotPoints projectedBoxFacePivotPoints, float offsetFromSnapSurface)
        {
            ...
            if (_objectSnapSurface.IsValid)
            {
                ...
                {
                    SnapObjectHierarchyPosition(hierarchyRoot, projectedBoxFacePivotPoints.ActivePoint, _objectSnapSurface.CursorPickPoint, projectedBoxFacePivotPoints, offsetFromSnapSurface);
                }
                else
                {
                    if (_objectSnapSurface.SurfaceType == SnapSurfaceType.GridCell && Settings.SnapCenterToCenterForXZGrid) SnapObjectHierarchyToCenterOfSnapSurface(hierarchyRoot, projectedBoxFacePivotPoints.CenterPoint, projectedBoxFacePivotPoints, offsetFromSnapSurface);
                    else
                    if (_objectSnapSurface.SurfaceType == SnapSurfaceType.ObjectCollider && Settings.SnapCenterToCenterForObjectSurface) SnapObjectHierarchyToCenterOfSnapSurface(hierarchyRoot, projectedBoxFacePivotPoints.CenterPoint, projectedBoxFacePivotPoints, offsetFromSnapSurface);
                    else SnapObjectHierarchyPosition(hierarchyRoot, projectedBoxFacePivotPoints.ActivePoint, _objectSnapSurface.GetSnapDestinationPointClosestToCursorPickPoint(), projectedBoxFacePivotPoints, offsetFromSnapSurface);
        ...
        {
            Vector3 closestObjPoint = Vector3.zero, closestHierarchyPt = Vector3.zero;
                            ...
                            foreach (Vector3 hierarchyVertex in hierarchyMeshOverlappedVerts)
                            {
                                foreach (Vector3 objectVertex in objectOverlappedVerts)
                                {
                                    ...
                                    if (distance < minDistance)
                                    {
                                        ...
                                        closestObjPoint = objectVertex;
                                        closestHierarchyPt = hierarchyVertex;
                                    }
                                }
                            }
                        }
                        ...
                        foreach (var meshObjInHierarchy in meshObjectsInHierarchy)
                        {
                            ...
                            foreach (Vector3 hierarchyVertex in hierarchyMeshOverlappedVerts)
                            {
                                foreach (Vector3 spriteCornerPt in spriteWorldCornerPoints)
                                {
                                    ...
                                    {
                                        minDistance = distance;
                                        closestObjPoint = spriteCornerPt;
                                        closestHierarchyPt = hierarchyVertex;
                                    }
                                }
                            }
                ...
                foreach (GameObject gameObject in nearbyObjects)
                {
                    ...
                    foreach (Vector3 hierarchyBoxPt in hierarchyBoxCornerPoints)
                    {
                        foreach (Vector3 objectMeshBoxPt in worldBoxCornerPoints)
                        {
                            ...
                            {
                                minDistance = distance;
                                closestObjPoint = objectMeshBoxPt;
                                closestHierarchyPt = hierarchyBoxPt;
                            }
                        }
                    }
                }
            ...
            if (minDistance < snapEpsilon) SnapObjectHierarchyPosition(hierarchyRoot, closestHierarchyPt, closestObjPoint, projectedBoxFacePivotPoints, 0.0f);
        }
