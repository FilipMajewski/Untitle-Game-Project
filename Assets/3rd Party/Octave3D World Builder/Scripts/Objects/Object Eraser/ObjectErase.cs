...
namespace O3DWB
{
    public static class ObjectErase
    {
        ...
        {
            HashSet<GameObject> parentsToErase = new HashSet<GameObject>(Octave3DWorldBuilder.Instance.GetParentsChildrenOfPivots(gameObjectsToErase));
            List<GameObject> objectsToErase = GetGameObjectsWhichCanBeErased(parentsToErase);
            PrepareGameObjectCollectionForEraseOperation(objectsToErase);
        ...
        public static void EraseEntireGameObjectHierarchy(GameObject gameObject)
        {
            ...
            GameObject firstParentWhichIsChildOfPivot = Octave3DWorldBuilder.Instance.GetFirstParentWhisIsChildOfAPivot(gameObject);

            if (firstParentWhichIsChildOfPivot != null) DestroyGameObject(firstParentWhichIsChildOfPivot, eraserSettings.AllowUndoRedo);
        }
        ...
        public static void EraseGameObject(GameObject gameObject)
        {
            ...
            ObjectEraserSettings eraserSettings = ObjectEraserSettings.Get();
            bool isPivotWorkingObject = Octave3DWorldBuilder.Instance.IsPivotWorkingObject(gameObject);
            ...
            {
                GameObject firstParentWhichIsChildOfPivot = Octave3DWorldBuilder.Instance.GetFirstParentWhisIsChildOfAPivot(gameObject);
                if (firstParentWhichIsChildOfPivot == gameObject)
                {
                    List<GameObject> immediateChildren = gameObject.GetImmediateChildren();
                    gameObject.MoveImmediateChildrenUpOneLevel(eraserSettings.AllowUndoRedo);
                    DestroyGameObject(gameObject, eraserSettings.AllowUndoRedo);
                ...
                else
                {
                    ...
                    if (ObjectQueries.IsGameObjectHierarchyEmpty(firstParentWhichIsChildOfPivot) && CanGameObjectBeErased(firstParentWhichIsChildOfPivot))
                        DestroyGameObject(firstParentWhichIsChildOfPivot, eraserSettings.AllowUndoRedo);
                    else
                    if (immediateParent.transform.childCount == 0 &&
                        ObjectQueries.IsGameObjectEmpty(immediateParent) &&
                        !Octave3DWorldBuilder.Instance.IsPivotWorkingObject(immediateParent)) DestroyGameObject(immediateParent, eraserSettings.AllowUndoRedo);
                }
            }
        }
