    ...
    public class Object3DMassEraseShape
    {
        ...
        public List<GameObject> GetOverlappedGameObjectsForEraseOperation()
        {
            ...
            if(_circleShape.HasCursorPickedObject())
            {
                ...
                if(!Octave3DWorldBuilder.Instance.IsPivotWorkingObject(pickedGameObject))
                {
                    GameObject firstParentWhichIsChildOfPivot = Octave3DWorldBuilder.Instance.GetFirstParentWhisIsChildOfAPivot(pickedGameObject);
                    if (pickedGameObject != null && firstParentWhichIsChildOfPivot != null)
                    {
                        Transform parentTransform = firstParentWhichIsChildOfPivot.transform;
