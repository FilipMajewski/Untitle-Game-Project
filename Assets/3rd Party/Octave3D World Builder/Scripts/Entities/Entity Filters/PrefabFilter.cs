...
namespace O3DWB
{
    ...
    public class PrefabFilter : ScriptableObject
    {
        ...
        public EntityNameFilter NameFilter
        {
            get
            {
                if (_nameFilter == null) _nameFilter = Octave3DWorldBuilder.Instance.CreateScriptableObject<EntityNameFilter>();
                return _nameFilter;
            }
        }
        ...
        private List<Prefab> FilterPrefabsByFilterProperties(List<INamedEntity> namedPrefabEntities)
        {
            ...
            {
                if (_filterByActiveTags.IsActive && prefab.TagAssociations.NumberOfAssociations != 0)
                {
