...
namespace O3DWB
{
    public abstract class SceneViewEventHandler
    {
        ...
        public void HandleSceneViewEvent(Event e)
        {
            ...
            switch(e.type)
            {
                ...
                case EventType.MouseDown:
                    ...
                    if (e.type == EventType.MouseDown && e.button == (int)MouseButton.Left) e.DisableInSceneView();
        ...
        protected virtual bool CanEventBeHandled(Event e)
        {
            ...
            if (e.type == EventType.MouseDown && !MouseButtonStates.Instance.IsOnlyThisMouseButtonDown((MouseButton)e.button)) return false;
        ...
        private bool HandleGeneralShortcutKeys(Event e)
        {
            ...
            if(e.type == EventType.KeyDown)
            {
                if(AllShortcutCombos.Instance.ActivateObjectPlacementGUI.IsActive())
                {
                    ...
                    UndoEx.RecordForToolAction(Octave3DWorldBuilder.Instance.Inspector);
                    Octave3DWorldBuilder.Instance.Inspector.ActiveInspectorGUIIdentifier = InspectorGUIIdentifier.ObjectPlacement;
                    return true;
                }
                else 
                if(AllShortcutCombos.Instance.ActivateObjectSelectionGUI.IsActive())
                {
                    e.DisableInSceneView();
                    UndoEx.RecordForToolAction(Octave3DWorldBuilder.Instance.Inspector);
                    Octave3DWorldBuilder.Instance.Inspector.ActiveInspectorGUIIdentifier = InspectorGUIIdentifier.ObjectSelection;
                    return true;
                }
                else
                if(AllShortcutCombos.Instance.ActivateObjectErasingGUI.IsActive())
                {
                    e.DisableInSceneView();
                    UndoEx.RecordForToolAction(Octave3DWorldBuilder.Instance.Inspector);
                    Octave3DWorldBuilder.Instance.Inspector.ActiveInspectorGUIIdentifier = InspectorGUIIdentifier.ObjectErase;
                    return true;
                }
            }
